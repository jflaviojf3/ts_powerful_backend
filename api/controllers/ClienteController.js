...
class ClienteController {
    ...
    static async pegaUmCliente(req, res){
        const {id} = req.params
        try {
            const umCliente = await database.Clientes.findOne({ where: {id_clientes:Number(id)}})
    ...
    static async atualizaCliente(req, res){
        ...
        try {
            await database.Clientes.update(atualizaCliente, { where: {id_clientes:Number(id)}})
            const umClienteAtualizado = await database.Clientes.findOne({ where: {id_clientes:Number(id)}})
    ...
    static async deletaCliente(req, res){
        const {id} = req.params
        try {
            await database.Clientes.destroy({ where: {id_clientes:Number(id)}})
