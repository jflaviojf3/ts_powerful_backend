...
describe.skip('Testes dos endpoints de tarefas do usuario', ()=>{
    ...
    it('Teste consulta tarefas do dia buscando dia atual, deve retornar 200', async ()=>{
        ...
        expect(res.statusCode).toEqual(200)
        expect(res.body).not.toHaveLength(1);
        expect(res.body[0].descricao).toEqual("Teste Automatizado Postman Incluido por Atualização")
        expect(res.body[0]).toMatchObject( expect.objectContaining({
            "id_tarefas": expect.any(Number),
            "id_projetos": expect.any(Number),
            "id_usuarios": expect.any(Number),
    ...
    it('Teste para atualizar uma tarefa do usuário 1, deve retornar 200', async ()=>{
        ...
        expect(res.body.descricao).toEqual("Teste Automatizado Jest alterado por Atualização")
        expect(res.body).toMatchObject( expect.objectContaining({
            "id_tarefas": expect.any(Number),
            "id_projetos": expect.any(Number),
            "id_usuarios": expect.any(Number),
    ...
    it('Teste para deletar tarefas de um usuario que foi atualizado, deve retornar 200', async ()=>{
        ...
        expect(res.statusCode).toEqual(200)
        expect(res.body.mensagem).toEqual(`Id tarefa ${id_tarefas_criado} deletado`)
    })
